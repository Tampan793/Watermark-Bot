...
# Support Group: @M4SK3R1N
# Bots Channel: @ViralTwitterHot
...
async def HelpWatermark(bot, cmd):
	...
	await cmd.reply_text(
		...
		parse_mode="Markdown",
		reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Developer", url="https://t.me/M4SK3R"), InlineKeyboardButton("Support Group", url="https://t.me/M4SK3R1N")], [InlineKeyboardButton("Bots Channel", url="https://t.me/ViralTwitterHot")], [InlineKeyboardButton("Source Code", url="https://github.com/M4SK3R1N")]]),
...
async def VidWatermarkAdder(bot, cmd):
	...
	if (int(file_size) > 2097152000) and (Config.ALLOW_UPLOAD_TO_STREAMTAPE is True) and (Config.STREAMTAPE_API_USERNAME != "NoNeed") and (Config.STREAMTAPE_API_PASS != "NoNeed"):
		...
		try:
			...
			print(f"Error: {e}")
			await editable.edit("Sorry, Something went wrong!\n\nCan't Upload to Streamtape. You can report at [Support Group](https://t.me/M4SK3R1N).")
...
async def button(bot, cmd: CallbackQuery):
	...
	if "refreshmeh" in cb_data:
			...
			try:
				...
				await cmd.message.edit(
					...
					reply_markup=InlineKeyboardMarkup(
						[
							[
								InlineKeyboardButton("🤖 Bergabung Updates Channel", url=invite_link.invite_link)
							],
							[
								InlineKeyboardButton("🔄 Mengulangi File 🔄", callback_data="refreshmeh")
							]
						]
					),
					parse_mode="markdown"
		...
		if Config.UPDATES_CHANNEL is None:
			await cmd.answer("Sorry Gan, Anda belum set Updates Channel!", show_alert=True)
			return
		try:
			user_id = cb_data.split("_", 1)[1]
			await bot.kick_chat_member(chat_id=Config.UPDATES_CHANNEL, user_id=int(user_id))
			await cmd.answer("User Banned dari Updates Channel!", show_alert=True)
		except Exception as e:
			await cmd.answer(f"Tidak dapat Ban Dia!\n\nError: {e}", show_alert=True)
